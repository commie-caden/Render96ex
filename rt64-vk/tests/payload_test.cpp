...
int main(int argc, char **argv) {
    ...
    SDL_Window *window = SDL_CreateWindow(
        "RT64 Vulkan — payload survives IgnoreHit?", SDL_WINDOWPOS_CENTERED,
    ...
    bindings[2].binding = 1;                    /* gProbe (u1 -> 1) */
    ...
    /* The probe buffer the anyhit writes and the raygen reads back. */
    RT64::BufferVK probeBuffer;
    if (!RT64::createBuffer(allocator, vk,
                            (VkDeviceSize)extent.width * extent.height * 16,
                            VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT |
                            VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
                            false, probeBuffer, error)) {
        std::fprintf(stderr, "  probe buffer: %s\n", error.c_str());
        return 1;
    }
    VkBufferViewCreateInfo probeViewInfo = {};
    probeViewInfo.sType = VK_STRUCTURE_TYPE_BUFFER_VIEW_CREATE_INFO;
    probeViewInfo.buffer = probeBuffer.buffer;
    probeViewInfo.format = VK_FORMAT_R32G32B32A32_SFLOAT;
    probeViewInfo.range = VK_WHOLE_SIZE;
    VkBufferView probeView = VK_NULL_HANDLE;
    if (!check(vkCreateBufferView(vk, &probeViewInfo, nullptr, &probeView),
               "CreateBufferView")) return 1;
    ...
    writes[2].pTexelBufferView = &probeView;
    vkUpdateDescriptorSets(vk, 3, writes, 0, nullptr);

    /* ----------------------------------------------------------- pipeline */
    RT64::ShaderLibraryVK shaders;
    VkShaderModule rtModule = VK_NULL_HANDLE;
    {
        const std::string path = shaderDir + "/TestPayloadRayTracing.spv";
    ...
    vkDestroyBufferView(vk, probeView, nullptr);
    probeBuffer.destroy(allocator);
    ...
    std::printf("\n  What to look for (red = payload, green = buffer readback):\n");
    std::printf("    YELLOW triangle -> both work; RT64's pattern is sound and\n");
    std::printf("                       the fault is elsewhere in the port.\n");
    std::printf("    RED triangle    -> the payload survives, but the raygen\n");
    std::printf("                       cannot see what the anyhit wrote to the\n");
    std::printf("                       buffer. That is the bug: RT64 reads its\n");
    std::printf("                       hit buffers straight after TraceRay.\n");
    std::printf("    GREEN triangle  -> the buffer write is visible but the\n");
    std::printf("                       larger payload struct is not surviving.\n");
